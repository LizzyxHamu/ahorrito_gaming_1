...
def index(request):
    lista_productos = Producto.objects.filter(activo=True).select_related('categoria').order_by('-creado_en')
...
def categoria(request, categoria_slug):
    ...
    lista_productos = Producto.objects.filter(categoria=categoria_obj, activo=True).order_by('nombre')
...
def wishlist_view(request):
    wishlist_items = request.user.wishlist_items.filter(activo=True)
